...
function PasswordResetRequests() {
  ...
  const loadRequests = async () => {
    try {
      ...
      const response = await api.get("accounts/password-reset-requests/", {
        headers: {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      await api.post(
        `accounts/password-reset-requests/${id}/approve/`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      await Swal.fire({
        icon: "success",
        title: "Approved",
        text: "Password reset email has been sent successfully to the user.",
        confirmButtonColor: "#198754",
      });
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      await api.post(
        `accounts/password-reset-requests/${id}/reject/`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <table>
          ...
          <tbody>
            {filteredRequests.length > 0 ? (
              ...
              <tr>
                <td colSpan="6">No password reset requests found.</td>
              </tr>
            )}
          </tbody>
        </table>
